...
class CreateClassifier:
    def __init__(self, embedding_path='../src/outputs/embeddings.pickle'):
        self.args = Namespace(det=0, embeddings='../src/outputs/embeddings.pickle', flip=0, ga_model='', gpu=0,
                              image_size='112,112', model='../insightface/models/model-y1-test2/model,0',
                              threshold=1.24)
    ...
    def train(self, classifier='svm'):
        x_train, x_test, y_train, y_test = train_test_split(self.embeddings, self.labels, stratify=self.labels,
                                                            test_size=0.3)
        print(x_train.shape)
        print(x_test.shape)
        if classifier == 'svm':
